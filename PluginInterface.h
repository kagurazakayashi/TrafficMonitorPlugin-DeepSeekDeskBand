 ...
 * @brief   TrafficMonitor 插件接口定义
 * @details 本文件定义了插件系统所需的三个核心接口：
 *          - IPluginItem：插件显示项目接口
 *          - ITMPlugin：插件主接口
 *          - ITrafficMonitor：主程序提供给插件的接口
 *          本文件来自 TrafficMonitor 上游仓库，插件开发者不应修改此文件。
 * @note    当前接口版本：API v7
 * @see     https://github.com/zhongyang219/TrafficMonitor
 * @copyright Copyright (C) by Zhong Yang 2021
 ****************************************************************************/
 ...
 * @class IPluginItem
 * @brief 插件显示项目接口
 * @details 每个显示项目（如时间、日期、网速等）都需要有一个类继承此接口。
 *          插件 dll 可以提供多个 IPluginItem 对象，每个对应一个显示项目。
 ****************************************************************************/
//插件显示项目的接口
class IPluginItem
{
 ...
 * @class ITMPlugin
 * @brief 插件主接口
 * @details 每个插件 dll 必须有一个类继承此接口，实现纯虚函数。
 *          类的实例通过导出的 TMPluginGetInstance() 函数返回给主程序。
 *          插件在此类中管理所有 IPluginItem 显示项目，并在此类中统一获取数据。
 ****************************************************************************/
///////////////////////////////////////////////////////////////////////////////////////////////////////
//插件接口
class ITMPlugin
...
public:
    ...
    struct MonitorInfo
    {
        unsigned long long up_speed{};              /**< 上传速度 */
        unsigned long long down_speed{};            /**< 下载速度 */
        int cpu_usage{};                            /**< CPU 使用率 */
        int memory_usage{};                         /**< 内存使用率 */
        int gpu_usage{};                            /**< GPU 使用率 */
        int hdd_usage{};                            /**< 硬盘使用率 */
        int cpu_temperature{};                      /**< CPU 温度 */
        int gpu_temperature{};                      /**< GPU 温度 */
        int hdd_temperature{};                      /**< 硬盘温度 */
        int main_board_temperature{};               /**< 主板温度 */
        int cpu_freq{};                             /**< CPU 频率 */
    };
 ...
 * @class ITrafficMonitor
 * @brief 主程序提供给插件的接口
 * @details 插件通过此接口可以获取主程序的监控数据、版本信息、DPI、配置目录等。
 *          此接口由主程序实现，插件获取此接口后可以调用其方法。
 ****************************************************************************/
///////////////////////////////////////////////////////////////////////////////////////////////////////
//主程序接口
class ITrafficMonitor
...
public:
     ...
     * @brief   获取当前语言的字符串资源
     * @param   key 字符串资源ID
     * @param   section 字符串资源在ini文件中的段名称
     * @return  对应的本地化字符串
     */
